...
class SoccerScene(Scene):
    def __init__(self):
        super().__init__()
        self._ball_obj = None
        self._robot_objs = {} # rid -> RenderObject
        self._dynamic_start_idx = 0

    def load(self, state):
        """ Construye la geometría estática y registra entidades dinámicas """
        self.objects = []
        ...
        # Guardar índice donde empiezan los objetos dinámicos
        self._dynamic_start_idx = len(self.objects)

        # 5. Registro de Entidades Dinámicas
        self._register_entities(state)
        self.initialized = True

    def _register_entities(self, state):
        # 1. Pelota
        if state.ball and not self._ball_obj:
            self._ball_obj = RenderObject(RenderObject.TYPE_CIRCLE, (1.0, 0.4, 0.0, 1.0))
            diameter = state.ball.radius * 2
            self._ball_obj.size = [diameter, diameter, diameter]
            
        # 2. Robots
        dark = 0.1
        for r in state.robots:
            rid = id(r)
            if rid not in self._robot_objs:
                color = (r.color[0]/255*dark, r.color[1]/255*dark, r.color[2]/255*dark, 1.0)
                self._robot_objs[rid] = RenderObject(RenderObject.TYPE_CYLINDER, color)
                diameter = r.radius * 2
                self._robot_objs[rid].size = [diameter, diameter, r.z_height]

    def update(self, state):
        """ Actualiza posiciones de entidades y reconstruye la lista de objetos activos """
        # Resetear lista de objetos a solo los estáticos
        self.objects = self.objects[:self._dynamic_start_idx]
        
        # 1. Pelota
        if state.ball:
            if not self._ball_obj: self._register_entities(state)
            self._ball_obj.position = [state.ball.x, state.ball.y, state.ball.radius]
            self.objects.append(self._ball_obj)
        ...
        for r in state.robots:
            ...
            rid = id(r)
            if rid not in self._robot_objs: self._register_entities(state)
            
            robot = self._robot_objs[rid]
